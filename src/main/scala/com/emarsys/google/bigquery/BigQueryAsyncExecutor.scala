...
trait BigQueryAsyncExecutor extends BigQueryExecutor {
  ...
  override implicit lazy val logger: LoggingAdapter = Logging(system, classOf[BigQueryAsyncExecutor])
  ...
  def handleJobResult(table: BqTableReference, jobResult: JobResult) = {
    ...
    if (errorResult.isEmpty) {
      ...
      logger.error("Job failed for table {} error: {} - {} - {}", table.table,
          errorResult.get.getMessage, errorResult.get.getReason, errorResult.get.getLocation)
    }
    errorResult.isEmpty
  }
}
