...
class JobStatusCheckerSpec extends TestKit(ActorSystem("testSystem"))
  with WordSpecLike
  with Matchers
  with BigQueryConfig
  with ImplicitSender {
  ...
  class TestPollingBigQuery extends BigQueryExecutor {
    ...
    override def execute[T](command: TableCommand[_])(implicit ec: ExecutionContext): Future[T] = {
      ...
      if (statusCommand.getJobId == "jobId" && statusCommand.getProjectId == "projectId") {
        ...
        val status = if (callIndex % 3 == 0) JobStatusChecker.STATUS_DONE else JobStatusChecker.STATUS_RUNNING
        Future.successful(createJob(status).asInstanceOf[T])
      } else {
        Future.failed(new Exception("Test command is invalid" + statusCommand.getJobId))
      }
    }
  }
