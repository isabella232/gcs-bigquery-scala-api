...
class QuerySourceSpec extends WordSpec with Matchers {
  ...
  "QuerySource" should {
    ...
    "be build from 2 tables" in {
      ...
      val joinedTable = JoinedTable(StandardTableSource(tableReference2), "t2", "t1.field = t2.field")
      val expected = s"${tableReference1.standardName} t1 LEFT JOIN ${tableReference2.standardName} t2 ON (t1.field = t2.field)"
    ...
    "be build from 3 tables" in {
      ...
      val baseTable = BaseTable(StandardTableSource(tableReference1), "t1")
      val joinedTable1 = JoinedTable(StandardTableSource(tableReference2), "t2", "t1.field = t2.field")
      val joinedTable2 = JoinedTable(StandardTableSource(tableReference3), "t3", "t2.field = t3.field")
      val expected = s"${tableReference1.standardName} t1 LEFT JOIN ${tableReference2.standardName} t2 ON (t1.field = t2.field) LEFT JOIN ${tableReference3.standardName} t3 ON (t2.field = t3.field)"
    ...
    "be build from 3 tables in legacy format" in {
      ...
      val joinedTable1 = JoinedTable(LegacyTableSource(tableReference2), "t2", "t1.field = t2.field")
      val joinedTable2 = JoinedTable(LegacyTableSource(tableReference3), "t3", "t2.field = t3.field")
      val expected = s"${tableReference1.legacyName} t1 LEFT JOIN ${tableReference2.legacyName} t2 ON (t1.field = t2.field) LEFT JOIN ${tableReference3.legacyName} t3 ON (t2.field = t3.field)"
      MultiTableSource(baseTable, joinedTable1, joinedTable2).show shouldEqual expected
    }
